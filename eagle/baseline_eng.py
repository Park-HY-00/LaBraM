"""
Baseline English: Within-Subject, Session-Agnostic English-Only Training

For each subject: All English sessions combined (task1, task2, task4)
Data split: 7:1.5:1.5 = train:valid:test
Training includes only English data with 15-class semantic labels
Purpose: Establish baseline performance for English-only within-subject learning
...
class BaselineEnglish:
    ...
    def create_braindecode_model(self, model_name):
        ...
        # Use shorter window for model compatibility
    ...
    def load_subject_english_data(self, subject_id):
        """Load all English session data for a specific subject using trackA.py approach"""
        print(f"  📂 Loading English data for subject {subject_id}...")
        ...
        if not eng_epochs:
            print(f"    ❌ No English epochs found for subject {subject_id}")
            return None, None
        
        # Combine all English epochs
        combined_epochs = mne.concatenate_epochs(eng_epochs)
        total_epochs = len(combined_epochs)
        
        print(f"    ✅ Total combined English epochs: {total_epochs}")
        print(f"    📊 English sessions loaded: {len(eng_session_info)}")
        
        return combined_epochs, eng_session_info
    ...
    def run_subject_analysis(self, subject_id):
        """Individual subject English-only baseline analysis"""
        print(f"\n{'='*70}")
        print(f"BASELINE ENGLISH: Subject-Dependent English-Only Analysis")
        ...
        # Load English data
        combined_epochs, session_info = self.load_subject_english_data(subject_id)
        
        if combined_epochs is None:
            print(f"❌ Insufficient English data for subject {subject_id}")
            return None
    ...
    def _train_and_evaluate_single_model(self, X_train, y_train, X_val, y_val, X_test, y_test, model_name):
            ...
            for batch_idx, (batch_X, batch_y_sem) in enumerate(train_loader):
                ...
                try:
                    ...
                    if model_name == 'EEGNet':
                        ...
                        # Use interpolation to change sampling rate
                        target_samples = 1500
                        ...
                        # TIDNet expects 4D input: (batch, 1, channels, time) with full time dimension
                        if len(batch_X.shape) == 3:  # (batch, channels, time)
                            model_input = batch_X.unsqueeze(1)  # (batch, 1, channels, time)
                        else:
                            model_input = batch_X
                        # No cropping for TIDNet - use full 1751 samples
                    else:
                        # Other braindecode models expect 4D input with cropped time dimension
                        if len(batch_X.shape) == 3:  # (batch, channels, time)
                            model_input = batch_X.unsqueeze(1)  # (batch, 1, channels, time)
                        else:
                            model_input = batch_X
                ...
                try:
                    ...
                    for i in range(0, len(X_val), self.batch_size):
                        ...
                        if model_name == 'EEGNet':
                            ...
                            # Resample to 500Hz/1500 samples
                            target_samples = 1500
        ...
        model_save_path = self.models_dir / f"best_model_eng_{self._current_subject_id}_{model_name}.pth"
        torch.save({
        ...
        history_save_path = self.training_history_dir / f"training_history_eng_{self._current_subject_id}_{model_name}.csv"
        ...
        test_results_save_path = self.test_results_dir / f"test_results_eng_{self._current_subject_id}_{model_name}.npz"
        np.savez(
    ...
    def run_all_subjects(self):
        """Execute analysis for all subjects with English data"""
        print(f"\n🚀 Starting Baseline English Analysis for All Subjects")
        
        # Get all subject IDs with English data
        subjects = set()
        
        # Find files for English data
        ...
        print(f"📋 Found {len(subjects)} subjects with English data: {subjects}")
    ...
    def run_first_subject_test(self):
        ...
        print(f"\n🧪 Starting Baseline English Analysis for First Subject (Testing Mode)")
    ...
    def _summarize_results(self, all_results):
        ...
        print(f"\n{'='*80}")
        print(f"BASELINE ENGLISH: BRAINDECODE MODELS COMPARISON SUMMARY")
        print(f"{'='*80}")
    ...
    def _save_results_to_file(self, all_results):
        ...
        txt_filename = self.results_dir / f"BaselineEnglish_Results_{timestamp}.txt"
        csv_filename = self.results_dir / f"BaselineEnglish_Results_{timestamp}.csv"
        
        with open(txt_filename, 'w', encoding='utf-8') as f:
            f.write("="*80 + "\n")
            f.write("BASELINE ENGLISH RESULTS\n")
            f.write("="*80 + "\n")
            f.write(f"Experiment Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Models Tested: {', '.join(self.model_names)}\n")
            f.write(f"Data Split: 7:1.5:1.5 (train:valid:test)\n")
            f.write(f"Language: English only\n")
            f.write("-"*80 + "\n\n")
...
if __name__ == "__main__":
    # Execute Baseline English
    baseline_eng = BaselineEnglish()
    
    print("🧪 Running Baseline English Analysis")
    print("🎯 Testing mode: First subject only")
    print(f"🤖 Models to test: {', '.join(baseline_eng.model_names)}")
    
    # Test with first subject
    # results = baseline_eng.run_first_subject_test()
    results = baseline_eng.run_all_subjects()
    
    if results:
        print("\n✅ Test completed successfully!")
        print("🔄 To run all subjects, use: baseline_eng.run_all_subjects()")
    else:
